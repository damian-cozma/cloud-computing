...
export default function Sessions() {
  ...
  const handleSelectGame = async (game) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/games/${game.id}/sessions`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleSaveSession = async (e) => {
    ...
    try {
      if (editingSessionId) {
        const res = await fetch(`${API_BASE}/api/sessions/${editingSessionId}`, {
          method: "PUT",
        ...
        const res = await fetch(`${API_BASE}/api/games/${selectedGame.id}/sessions`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(sessionForm),
        });
